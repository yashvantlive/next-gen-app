      ...
      {data.map((exp, index) => (
        <div key={exp.id} className="p-5 border border-gray-200 rounded-xl bg-white shadow-sm space-y-4 group hover:border-blue-200 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor={`exp-role-${exp.id}`} className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Role / Position</label>
              <input 
                id={`exp-role-${exp.id}`}
                name={`exp-role-${exp.id}`}
                type="text" 
                value={exp.role || ''} 
            ...
            <div>
              <label htmlFor={`exp-company-${exp.id}`} className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Company / Organization</label>
              <input 
          ...
          <div>
            <label htmlFor={`exp-duration-${exp.id}`} className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Duration</label>
            <input 
              id={`exp-duration-${exp.id}`}
              name={`exp-duration-${exp.id}`}
              type="text" 
              value={exp.duration || ''} 
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
               <label htmlFor={`exp-bullet-${exp.id}-0`} className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-0.5">Key Responsibilities</label>
               <button 
            ...
            <div className="space-y-2">
              {(exp.description ? exp.description.split('\n') : []).map((bullet, bIndex) => (
                <div key={bIndex} className="flex items-center gap-2">
                   ...
                   <input                      id={`exp-bullet-${exp.id}-${bIndex}`}
                     name={`exp-bullet-${exp.id}-${bIndex}`}                     type="text"
                     value={bullet}
